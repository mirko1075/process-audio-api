...
def register():
    ...
    try:
        ...
        if User.query.filter_by(email=email).first():
            return jsonify({'error': 'Email already registered'}), 4
        ...
        return jsonify({'error': 'Registration failed'}), 5
...
def login():
    ...
    try:
        ...
        if not email or not password:
            return jsonify({'error': 'Email and password required'}), 4
        ...
        return jsonify({'error': 'Login failed'}), 5
...
def create_api_key():
    ...
    try:
        ...
        return jsonify({'error': 'Failed to create API key'}), 5
...
def delete_api_key(key_id):
    ...
    try:
        ...
        return jsonify({'error': 'Failed to deactivate API key'}), 5
